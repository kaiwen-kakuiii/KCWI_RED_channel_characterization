...
def _merge_yaml_lists():
    """Cubes named in pipeline/config join the lists this file already carries."""
    if getattr(_merge_yaml_lists, "done", False):
        return
    sys.path.insert(0, ROOT)
    from pipeline.common import configured_lists
    drops, flags = configured_lists()
    for key, value in drops.items():
        DROP.setdefault(key, value)
    for key, value in flags.items():
        FLAG.setdefault(key, value)
    _merge_yaml_lists.done = True


def sensfunc_paths(grating):
    """Historical night trees, plus new cubes under reductions/.

    A file under reductions/ whose basename already exists in the historical
    tree is skipped, so re-reducing a published night does not average the
    cube with itself.
    """
    suffix = SUFFIX[grating]
    historical = sorted(glob.glob(os.path.join(
        RESULTS, grating, "*", "keck_kcrm_*", f"*_sens_{suffix}.fits")))
    seen = {os.path.basename(path) for path in historical}
    extra = []
    for path in sorted(glob.glob(os.path.join(
            ROOT, "reductions", grating, "*", "pypeit_run",
            "keck_kcrm_*", f"*_sens_{suffix}.fits"))):
...
def composite(groups, path, csv_path):
    ...
    with open(os.path.join(RESULTS, "composite_summary.csv"), "w",
              newline="") as fh:
        wtr = csv.writer(fh)
...
def main():
    ...
    ap.add_argument("--outdir", default=os.path.join(RESULTS, "blaze_plots"))
    args = ap.parse_args()
    ...
    rows = superblaze(groups, os.path.join(RESULTS, "kcrm_superblaze.png"))
    comp = composite(groups, os.path.join(RESULTS, "kcrm_composite.png"),
                     os.path.join(RESULTS, "composite_by_grating.csv"))
    trim_report(cubes, os.path.join(RESULTS, "trim_summary.csv"))
    edge_residuals(os.path.join(RESULTS, "edge_check.csv"))

    # Per-cube CSV: the table in GRATING_SUMMARY.md is generated from this, so
    # the document and the figures cannot drift apart.
    csv_path = os.path.join(RESULTS, "blaze_summary.csv")
    with open(csv_path, "w", newline="") as fh:
        wtr = csv.writer(fh)
    ...
    with open(os.path.join(RESULTS, "blaze_peaks.csv"), "w", newline="") as fh:
        wtr = csv.writer(fh)
