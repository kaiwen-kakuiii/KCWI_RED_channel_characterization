...
The prose lives in `results/grating_summary.template.md`, with a
...
Usage:
    python pipeline/utils/blaze_tables.py --render   # rewrite results/GRATING_SUMMARY.md
...
def build():
    rows = list(csv.DictReader(open(os.path.join(RESULTS, "blaze_summary.csv"))))
    ...
    comp = {r["grating"]: r for r in csv.DictReader(
        open(os.path.join(RESULTS, "composite_summary.csv")))}
    trim = {r["grating"]: r for r in csv.DictReader(
        open(os.path.join(RESULTS, "trim_summary.csv")))}
    out = {}
    ...
    edge = {r["grating"]: r for r in csv.DictReader(
        open(os.path.join(RESULTS, "edge_check.csv")))}
...
TEMPLATE = os.path.join(RESULTS, "grating_summary.template.md")
DOC = os.path.join(RESULTS, "GRATING_SUMMARY.md")
