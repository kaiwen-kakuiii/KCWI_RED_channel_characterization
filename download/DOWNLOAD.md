...
```
download/kcwi_stds_selected.txt          40 standards, an observer's starlist
        |
        |  fetch_std_red.py     one ADQL cone query, label each frame with its star
        v
download/outputKC/std_red_matched.csv    1307 red science frames, 27 standards
        |
...
[pipeline/TUTORIAL.md](../pipeline/TUTORIAL.md) takes it from there. The
reduction notes for the published nights are in
[archive/PYPEIT.md](../archive/PYPEIT.md).
...
python download/fetch_std_red.py

# 2. see which star covers which grating                             (offline)
python download/analyze_gratings.py

# 3. see what one night would give you, download nothing             (offline*)
python download/download_std_red.py --stars feige110 --same-night --nights 2023-12-17 --dry-run

# 4. actually fetch it, science + same-night calibrations
python download/download_std_red.py --stars feige110 --same-night --nights 2023-12-17
...
`download/outputKC/std_red_matched.csv`, is checked in, so steps 2 and 3 work straight
...
| -------------- | ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `STARLIST`     | `download/kcwi_stds_selected.txt` | the standards to look for                                                                                                                                                  |
...
| `--cookie PATH`      | `download/outputKC/koa_cookie.txt`                       | KOA login cookie, for proprietary frames                                                          |
...
cover `--require`. Lists are cached in `download/outputKC/caliblists` and in whatever an
...
Everything except the FITS lands in `download/outputKC/`.
...
```bash
python download/fetch_std_red.py                 # once; ~9 s, writes std_red_matched.csv
cd KCWI_RED_channel_characterization
python download/analyze_gratings.py              # which star covers which grating
```

**Check what a selection would give you**

```bash
python download/download_std_red.py --stars feige110 --same-night --dry-run
python download/download_std_red.py --stars feige110 --same-night \
